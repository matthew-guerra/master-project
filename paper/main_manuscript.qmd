...
The 20th century was marked by increased rural-to-urban Indigenous migration, and according to the 2000 U.S. Census approximately 66% of all Indigenous people live in cities [@howard2013; @ogunwole2006]. Scholars have attributed this pattern of migration and relocation to assimilation policies by the United States, which aimed to, "Kill the Indian, save the man" through relocation and the outlaw of Indigenous languages and cultural and religious practices [@adams1995education]. Despite these efforts, the relocation of thousands of Indigenous individuals to urban centers led to the emergence of new forms of Indigenous identity and political consciousness. In many cities, relocated individuals found themselves in shared experiences of marginalization and cultural dislocation, leading to the development of inter-tribal and pan-Indian identities in addition to increased rates of interracial unions [@krouse1999kinshipa; @rosenthal2012a]. These urban Indigenous communities established organizations focused on mutual aid, social services, education, and political activism. It was within these very spaces that the Red Power Movement gained momentum, ultimately giving rise to the American Indian Movement (AIM). In response to systemic anti-Indigenous policies, discrimination, and police violence, Indigenous activists mobilized, advocating for self-determination, tribal sovereignty, and Indigenous rights on a national scale [@kent-stoll2024a; @weaver2012a]. This article aims to illuminate this process by asking: What role does geography and ethnicity play into racial identification for Indigenous, multiracial respondents? This article engages with settler-colonialism, racial formation theory, and socio-historical context as a guiding framework to interpret findings.

<!--# The first two paragraphs are too in the weeds in my view and read like part of the background parts of an argument section. I think talking about assimilation and racial identification more broadly would work very well here-Ryan -->

Examining the history of Indigenous relocation highlights a need to understand the role of geographic settings—whether metropolitan, rural, or a homeland—in shaping racial identity and classification[^1]. Investigating these dynamics is essential to understanding how Indigenous peoples negotiate their identity through limiting racial categories like AIAN, and how historical and structural forces continue to influence how Indigenous identity is constructed, recognized, and legitimized through bureaucratic means like the American Community Survey (ACS).<!--# "Good start but add more about data and findings"-Ryan -->v<!--# "I'm still confused on what your stance/ arguments are – I think just include a clear thesis statement in the last sentence of your last paragraph"-Jordan -->
...
<!--# The homeland area is added as a control variable. However, in the regression tables, it seems as if it is an independent variable since it is not added with other control variables in the final model. It is also a significant predictor and relevant for your theoretical framework so moving it to your independent variables might be more appropriate.;Metropolitan status is an independent variable, yet the categories are not very self-explanatory. What does unknown status mean? How that might have impact the findings?;The author acknowledges that many reservation areas are located in rural areas, which raises questions on a potential collinearity between homeland and rural/metropolitan status. The homeland variable’s role in the framework could be specified further;Age is statistically significant, but the coefficients are so small that it does not practically mean anything, which should be noted in the results;Small sample size and unstable coefficient among Asian-Indigenous respondents the interpretations of statistical significance should be more cautious-Sena -->
...
The predictor variables are homeland area (binary), educational attainment (less than HS diploma, HS diploma, some college, 4-year degree or more), gender (binary), Hispanic origin (binary) <!--# "if it is in the title, it should be IV"-Ryan -->, and age (continuous, 18+).

## **Results** <!--# "Univariate statistics including figures like figure 1?-"-Ryan -->

<!--# The interpretations of likelihood and probabilities confuse the reader. The reported coefficients are log-odds, not direct probabilities and percentage changes. Presenting odds ratios directly or computing predicted probabilities in a consistent manner would help with this;When findings are interpreted in the discussion part, there is little engagement with the previously established theories. The author has a strong theoretical discussion in the first sections, yet the theoretical links are mostly lacking and a connection back to the previous debates would be helpful for situating the findings within the literature. In addition to theoretical implications of the findings, future directions and limitations could be added to this section.-Sena -->

My research question focuses on understanding the relationship between metropolitan status and the racial decision-making process for those with mixed Indigenous ancestry, in addition to determining how rurality affects the predictive power of the Homeland variable. <!--# "Clarify research questions in the introduction. I missed this one"-Ryan --> I use the focal predictor variables to answer this question by identifying the relationship between the dependent and independent variables and the predictor variables. Each table presents coefficients, standard errors, and significance levels for the multinomial logistic regression models predicting race response with the predictor variables. Non-Indigenous Race alone is the reference group for each model, as the goal of the article is to determine how rurality and the Homeland variable influence a move towards Indigeneity or multiracial identity. @tbl-1 represents respondents with White and Indigenous ancestry, @tbl-2 represents respondents with Black and Indigenous ancestry, and @tbl-3 represents respondents with Asian and Indigenous ancestry. The model results compare the predicted race response of Indigenous Alone and Multiracial to non-Indigenous Race Alone with a separate table for each ancestry group. The base models of each table look at racial responses in relation to metropolitan status, the subsequent model adds Homeland status and the final model adds educational attainment, gender, Hispanic origin <!--# "Re-title"-Ryan -->, and age. This setup creates three tables with three models each for a total of 9 models.

<!--# The regression tables do not state the reference categories are not specified-Sena -->
...
### **Hispanic Origin** <!--# "Again, move forward or remove from title"-Ryan -->
...
## **Discussion** <!--# "I’m sure you already know this feedback: connect each ﬁnding to your literature review and your argument;I’m not sure if your argument is that the ongoing history of settler colonialism explains why people have such di`erent understandings of their racial identity, hence why someone’s geographical location inﬂuences their racial identity; Or is the literature on settler colonialism and racial identity more so to situate how you use your quantitative data to properly represent historically marginalized communities?"-Jordan-->
...
Since few factors seemed to deter those with both Asian and Indigenous ancestry from identifying as Asian alone, @fig-1 <!--# "It is pretty unusual to include a figure in the discussion. I think that you could move this into earlier sections"-ryan --> was created to identify the breakdown of how they identify. Surprisingly Asian/Indigenous respondents made up the vast majority of respondents that indicated a multiracial identity despite the findings above. More research on multiracial Asians is needed to identify what factors could explain this finding.
