...
Additionally, a discussion about Hispanic or Latino Origin would not complete without discussing the role of *mestizaje. Mestizaje*, or mixed is a concept that plays a major role in the racial dynamics of many Latin American countries. It both privileges whiteness and aims to assimilate Indigeniety through the use of the mestizo identity [@globalr2018]. It does this by making all members the same race, descendants of Spanish settlers and those Indigenous to the area. Essentially they deny racial differences within the population. For example, many believe that to be Mexican is to be mestizo and those who are not mestizo do not fit in to the racial scheme of the area.
...
DATA$race_response <- forcats::fct_recode(DATA$race_response,
                                               "Indigenous" = "Indigenous, Alone", "Consistent" = "Multiracial")
...
The findings from this study revealed patterns of Indigenous identity that align with previous research while also presenting unique deviations. Analysis of metropolitan status across the three multiracial ancestry groups identified distinct patterns in identity responses. In Model 1, metropolitan residence was negatively associated with identifying as Indigenous alone and positively associated with multiracial identification. However, when the Homeland variable was introduced, there was a 47% increase in the likelihood of Indigenous-alone identification among metropolitan respondents, suggesting that rural residence alone does not significantly increase Indigenous-alone identity without the presence of a Homeland connection.
