...
DATA <- DATA %>%
  mutate(
    Rancestry1s = ifelse(Rancestry1 %in% c("European", "North African and Middle Eastern"), "White",
                         ...
                         ifelse(Rancestry1 %in% c("American Indian"), "AIAN",
...
DATA <- DATA %>%
  mutate(
    Rancestry2s = ifelse(Rancestry2 %in% c("European", "North African and Middle Eastern"), "White",
                         ...
                         ifelse(Rancestry2 %in% c("American Indian"), "AIAN",
...
DATA$RaceC <- factor(ifelse(DATA$RaceD == 100, "White",
                  ...
                  ifelse(DATA$RaceD == 802, "White and AIAN",
                  ifelse(DATA$RaceD == 830, "Black and AIAN",
                  ifelse(DATA$RaceD >= 850 & DATA$RaceD <= 854,
                         "AIAN and Asian",
                         NA)))))))))

DATA$RaceC <- factor(DATA$RaceC, levels = c("White", "Black", "AIAN", "Pacific Islander", "Asian", "White and AIAN","Black and AIAN","AIAN and Asian"))
```

```{r}
#filter for selected cases
DATA$selected_cases <- (((DATA$Rancestry1s == "AIAN") & 
                      !(DATA$Rancestry2s == "AIAN")) |
  ((DATA$Rancestry2s == "AIAN") & !(DATA$Rancestry1s == "AIAN"))) & 
  !(DATA$Rancestry1s == "PI") & !(DATA$Rancestry2s == "PI") & 
  !(DATA$Rancestry1s == "Latino") & !(DATA$Rancestry2s == "Latino")

DATA <- subset(DATA, 
  ((Rancestry1s == "AIAN") & !(Rancestry2s == "AIAN") |
     (Rancestry2s == "AIAN") & !(Rancestry1s == "AIAN")) & 
    !(Rancestry1s == "PI") & 
    !(Rancestry2s == "PI") & 
    !(Rancestry1s == "Latino") & 
    !(Rancestry2s == "Latino")
...
DATA(temp$Rancestry1s, temp$Rancestry2s, exclude=NULL)

DATA <- DATA |>
  mutate(race_ancestry = case_when(
    Rancestry1s == "AIAN" & Rancestry2s == "White" ~ "AIAN/White",
    Rancestry1s == "White" & Rancestry2s == "AIAN" ~ "AIAN/White",
    Rancestry1s == "AIAN" & Rancestry2s == "Black" ~ "AIAN/Black",
    Rancestry1s == "Black" & Rancestry2s == "AIAN" ~ "AIAN/Black",
    Rancestry1s == "AIAN" & Rancestry2s == "Asian" ~ "AIAN/Asian",
    Rancestry1s == "Asian" & Rancestry2s == "AIAN" ~ "AIAN/Asian",
  ))
...
DATA <- DATA |>
  mutate(race_response = case_when(
    RaceC == "AIAN" ~ "AIAN Alone",
    race_ancestry == "AIAN/Asian" & RaceC == "Asian" ~ "Other Race Alone",
    race_ancestry == "AIAN/White" & RaceC == "White" ~ "Other Race Alone",
    race_ancestry == "AIAN/Black" & RaceC == "Black" ~ "Other Race Alone",
    race_ancestry == "AIAN/Asian" & RaceC == "AIAN and Asian" ~ "Multiracial",
    race_ancestry == "AIAN/White" & RaceC == "White and AIAN" ~ "Multiracial",
    race_ancestry == "AIAN/Black" & RaceC == "Black and AIAN" ~ "Multiracial",
    TRUE ~ "Inconsistent"
  ))

DATA <- DATA |>
  mutate(race_respone = factor(race_response,
                               levels=c("Other Race Alone", "AIAN Alone",
...
DATA <- subset(DATA, select=c("Year","Age","Gender","MetroC","HomelandP",
                       "Hispanic", "RaceC", "EducA","RaceC","selected_cases"))
DATA
```
