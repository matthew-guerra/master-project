...
```{r}
#recode into big racial categories
DATA <- DATA %>%
  mutate(
    Rancestry1s = factor(
      case_when(
        Rancestry1 %in% c(1, 8) ~ "White",
        Rancestry1 %in% c(2, 9, 7) ~ "Black",
        Rancestry1 == 3 ~ "AIAN",
        Rancestry1 == 5 ~"Pacific Islander",
        Rancestry1 %in% c(4, 10) ~ "Asian",
        Rancestry1 == 6 ~ "Latino"
      ),
      levels = c("White", "Black", "AIAN","Pacific Islander", "Asian", "Latino")
    )
  )
```
